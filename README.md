...
------------------
The evaluator is a Java program. To run it, download the [compiled jar](https://github.com/recsyschallenge/RSChallengeEval/releases/download/v0.12/rscevaluator-0.12-jar-with-dependencies.jar) and execute it like:
```bash
java -jar rscevaluator-0.12-SNAPSHOT-jar-with-dependencies.jar "/path/to/testfile.dat" "/path/to/predictions.dat"
```
...
Compiling:
```bash
javac -cp ./rscevaluator-0.12-jar-with-dependencies.jar Example.java
```
Executing:
```bash
java -cp ./rscevaluator-0.12-jar-with-dependencies.jar:. Example
```
